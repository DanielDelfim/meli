...
CUSTOS_PATH = BASE / "tokens" / "custos.json"
...
def render_consolidado_financeiro():
    ...
    # Carrega custos e tenta criar a coluna de período
    try:
        df_custos = pd.read_json(CUSTOS_PATH)
        if "mes_competencia" not in df_custos.columns:
            st.error("❌ A coluna 'mes_competencia' não está presente no arquivo de custos.")
            st.stop()
        df_custos['period'] = pd.to_datetime(df_custos['mes_competencia'], format="%Y-%m").dt.to_period('M')
    except Exception as e:
        st.error(f"Erro ao processar 'mes_competencia': {e}")
        st.stop()
    ...
    sel = st.selectbox("🗓️ Mês de Competência", labels, key="select_mes_publicidade")
    ...
    # Busca custos e MCP do mês selecionado
    record = df_custos[df_custos['period'] == period]
    if not record.empty:
        dados_mes = record.iloc[0].to_dict()
        mcp_sp = float(dados_mes.get("mcp_sp", 0.0))
        mcp_mg = float(dados_mes.get("mcp_mg", 0.0))

        # Soma todas as despesas numéricas do mês (exceto campos ignorados)
        campos_ignorar = ["mes_competencia", "custo_total", "publicidade", "mcp_sp", "mcp_mg"]
        custo_fixo_total = sum(
            v for k, v in dados_mes.items()
            if isinstance(v, (int, float)) and k not in campos_ignorar
        )
    else:
        mcp_sp = 0.0
        mcp_mg = 0.0
    ...
    margem_contribuicao_sp = receita_sp * mcp_sp
    margem_contribuicao_mg = receita_mg * mcp_mg
    margem_total = margem_contribuicao_sp + margem_contribuicao_mg

    # Lucro ou prejuízo com base em custo fixo total
    lucro_prejuizo = margem_total - custo_fixo_total
    ...
    col3.metric("MCP SP", f"{mcp_sp * 100:.2f}%")
    col4.metric("MCP MG", f"{mcp_mg * 100:.2f}%")

    st.markdown("### 💰 Margens e Custo")
    col5, col6 = st.columns(2)
    col5.metric("Margem Total (SP + MG)", f"R$ {margem_total:,.2f}")
    col6.metric("Custo Fixo Total", f"R$ {custo_fixo_total:,.2f}")
